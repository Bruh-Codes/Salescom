...
<template>
  <div
    class="bg-n-popover text-n-popover-foreground border border-n-border absolute rounded-md z-50 flex flex-col min-w-[136px] shadow-md pt-2 overflow-hidden"
  >
    <div v-if="showSearch" class="relative shrink-0 px-2 mb-2">
      ...
      <input
        ...
        "
        class="reset-base w-full h-8 py-2 ltr:pl-10 ltr:pr-2 rtl:pl-2 rtl:pr-10 text-sm outline outline-1 outline-n-input focus:outline-n-ring border-none rounded-md bg-n-background text-n-foreground placeholder:text-n-muted-foreground"
    ...
    <div class="flex flex-col gap-2 overflow-y-auto min-h-0 px-2 pb-2">
      <template v-if="hasSections">
        ...
        >
          <p
            v-if="section.title"
            class="px-2 py-2 text-xs mb-0 font-medium text-n-muted-foreground uppercase tracking-wide sticky top-0 z-10 bg-n-popover truncate min-w-0"
          >
            {{ section.title }}
          </p>
          <div
          ...
          <button
            ...
            type="button"
            class="inline-flex items-center justify-start w-full h-8 min-w-0 gap-2 px-2 py-1.5 transition-colors duration-200 border-0 rounded-sm z-60 hover:bg-n-accent hover:text-n-accent-foreground disabled:cursor-not-allowed disabled:pointer-events-none disabled:opacity-50"
            :class="{
              'bg-n-accent text-n-accent-foreground': item.isSelected,
