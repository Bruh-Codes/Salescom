...
<template>
  <TeleportWithDirection to="body">
    <dialog
      ref="dialogRef"
      class="w-full transition-all duration-200 shadow-lg rounded-lg bg-transparent"
      :class="[
        maxWidthClass,
        positionClass,
    ...
    >
      <OnClickOutside @trigger="handleClickOutside">
        <form
          ...
          class="flex flex-col w-full h-auto gap-6 p-6 overflow-visible text-start align-middle border border-n-border bg-n-popover text-n-popover-foreground shadow-lg rounded-lg"
          @submit.prevent="confirm"
          @click.stop
        >
          <div v-if="title || description" class="flex flex-col gap-2">
            <h3 class="text-lg font-semibold leading-none text-n-foreground">
              {{ title }}
            </h3>
            <slot name="description">
              <p
                v-if="description"
                class="mb-0 text-sm text-n-muted-foreground"
              >
                {{ description }}
              </p>
            </slot>
