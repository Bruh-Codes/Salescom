...
const inputOutlineClass = computed(() => {
  switch (props.messageType) {
    ...
    default:
      return 'outline-n-input hover:outline-n-ring/60 disabled:outline-n-input focus:outline-n-ring';
  }
});
    ...
    <input
      ...
      :max="
        ['date', 'datetime-local', 'time', 'number'].includes(type)
          ...
          : undefined
      "
      class="block w-full reset-base text-sm !mb-0 outline outline-1 border-none border-0 outline-offset-[-1px] rounded-md bg-n-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-n-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 text-n-foreground shadow-sm transition-colors duration-200 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
      @input="handleInput"
      @focus="handleFocus"
      @blur="handleBlur"
